...
the slot would land after applying the player's movement, while `wrapPosition()` only folds it back inside the grass field:
...
inside `wrapPosition()` shifts it by one full tile. `step()` turns that jump into the `0/1` flag I need, and the comparison still
works if a larger movement crosses more than one tile at once.
...
Have you ever tried to balance a light switch exactly in the middle? Well, I surely did, and of course I exactly as one might
expect: I couldn't.
...
There is no longer one switching point I could somehow balance on. At `10`, moving back to `11` is not enough to switch again, and
the same happens at `20` when moving back to `19`. That point does not exist, which finally explains why balancing a light switch
was a lost battle from the beginning.
...
```typescript
const passesStochasticThinning =
  VegetationSsboUtils.computeStochasticKeep(/* ... */);
...
If(isPotentiallyVisible, () => {
  ...
  If(isVisible, () => {
    // ...terrain height, trail, wind, shadow and atomic compaction
  });
});
```
...
shadow and atomic work. Sampling the grass map before the first gate would make all 1M blades pay for it instead.
...
I already had the previous visibility and the current stored scale, so the missing reset ended up being quite small:
...
You may have noticed that the code above now calls `computeGrassScale()` instead of `computeAlpha()`. Underneath, it is still the
exact same sample from the same green channel of `grassMap`. The value had always been continuous between `0` and `1`; I was simply
reducing it to a yes or no instead of using it to control the height.
